...
struct Rotators {
  ...
  int FindMedian3xLeaker(double window) {
    ...
    static const double kMagic = 2.7561870637459678 + 0.0001 * atof(getenv("VAR256"));
    
    static const double kRound = -0.0062803032837967804 + 0.01 * atof(getenv("VAR257"));
  ...
  Rotators(int num_channels, const float sample_rate) {
    ...
    double cp = 0.83712437948019858 + atof(getenv("VAR0"));
    double cm = 1.1018121433506995 + atof(getenv("VAR1"));
    for (size_t i = 0; i < kNumRotators; ++i) {
...
#define BINAURAL
	  ...
	  }
	  {
	    ...
	    static double kMul = 11.363882268949252 + atof(getenv("VAR258"));
