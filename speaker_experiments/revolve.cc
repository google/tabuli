...
struct Rotators {
  ...
  int FindMedian3xLeaker(double window) {
    ...
    static const double kMagic = 2.7561865280292395 + 0.0001 * atof(getenv("VAR256"));
    
    static const double kRound = -0.0063630066863939461 + 0.01 * atof(getenv("VAR257"));
  ...
  Rotators(int num_channels, const float sample_rate) {
    ...
    // Intuitively this should be 2 * M_PI / kSampleRate, but this
    // formalism rotates once when signals are placed into the integrators and
    // again when values are read, so we need to drop 2 to get right values.
    // Due to same we need to add sqrts for gains.
    static const float kHzToRad = M_PI / kSampleRate;
    //    static const double kWindow = 0.9988311560491858
    //    static const double kWindow = 0.99963; // 72129.93163818851 (0.9996)->60697->
    static const double kWindow = 0.9995;
    ...
    double cp = 0.6752635726771451 + atof(getenv("VAR0"));
    double cm = 1.1038141406720734 + atof(getenv("VAR1"));
    for (size_t i = 0; i < kNumRotators; ++i) {
      float f = M_PI * 2 * ((cp + cm * delay[i]) * FreqAve(i) / 48000.0);
...
struct MultiChannelDriverModel {
  ...
  void Convert(float *p, size_t n) {
    ...
    for (int k = 0; k < n; ++k) {
      const float kHighPassFreq = 25;
      const float kAve = pow(0.5, 1 / (48000.0/kHighPassFreq)); 
      ave[k] *= kAve;
      ave[k] += (1.0 - kAve) * p[k];
...
#define BINAURAL
	  ...
	  }
	  {
	    ...
	    static double kMul = 10.94883520967808 + atof(getenv("VAR258"));
